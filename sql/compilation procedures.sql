...
begin
    if(name is null) then
        raise_application_error(-20001, 'the name is null for procedure createCompilation');
    end if;
    insert into compilations (name, creation_time, duration) values (name, localTimeStamp(0), 0);
    id := compilation_id_seq.currval;
end;
/
create or replace procedure addSampleToCompilation(
